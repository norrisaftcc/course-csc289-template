# DataMan — Product & Technical Reference
...
This is your single reference for everything about DataMan — what it is, how
each game mode works, what the database looks like, how the adaptive difficulty
algorithm works, and what Flask routes you'll need to build.

You don't need to memorize this. You need to **read it once** so you know
what's here, and then **come back to it** when you're building a specific
feature and need the details.

The companion document `G4_Project_Planning.md` covers the sprint schedule,
grading criteria, workflow, and getting started checklist. This document covers
the *product itself*.

---

## Part 1: What Is DataMan?

DataMan is a web application where users practice math through short game
sessions. After each session, the application saves their results and adjusts
the difficulty of future problems based on how they've been doing.

That's the entire product in two sentences. Everything below is just the details
of how that works.

### The Core Loop

Every interaction with DataMan follows this pattern:

```
 1. User logs in
 2. User picks a game mode from the dashboard
 3. User plays a session (15 questions, or 60 seconds, depending on mode)
 4. Application saves the results
 5. Application adjusts difficulty for next time
 6. Dashboard updates to show progress
 7. User comes back tomorrow (or in five minutes)
```

### What Makes It a "Product" and Not Just a "Quiz App"

Three things separate DataMan from a homework assignment:

**Persistence.** Your results don't disappear when you close the browser.
The database remembers every session, every answer, every timestamp. When you
come back tomorrow, your streak is intact and your dashboard shows your history.

**Adaptation.** The application pays attention. If you're getting 95% of the
multiplication problems right, it starts giving you harder ones. If you're
struggling with division, it eases up. This happens automatically, behind the
scenes, without the user doing anything.

**Progress visibility.** The dashboard doesn't just say "you scored 12/15."
It shows your accuracy over the past week. It shows your streak. It shows which
modes you're strong in and which ones need work. It tells a *story* about whether
you're improving.

---

## Part 2: The Four Game Modes
...
**The idea:** An equation appears on screen with a proposed answer. The user
decides: is it right, or is it wrong?
...
In this case, 24 + 17 *does* equal 41, so "Right" is correct.
...
**The idea:** The application picks a secret number. The user guesses. After
each guess, they're told "higher" or "lower." Goal: find the number in as few
guesses as possible.
...
Students don't need to know this to play, but it's a nice thing to surface
on the results screen: "You found it in 5 guesses! (Optimal: 7 for this range)"
...
**The idea:** A 60-second countdown timer. Math problems appear one at a time.
Answer as many as you can before time runs out. This is the most "game-like"
mode — fast, exciting, and great for demos.
...
unplayable. The server validates the final results, but the timing is client-side.
...
**The idea:** An equation has a blank — a missing number or a missing operator.
The user figures out what goes in the blank.
...
If you haven't used SQLAlchemy before, the [Flask-SQLAlchemy quickstart](https://flask-sqlalchemy.palletsprojects.com/) is the place to start. It takes about 30 minutes to read and will save you hours.

---

## Part 4: Adaptive Difficulty
...
struggling, problems get easier. If they're in the middle, things stay the same.
...
Each of the other modes has its own tier parameter table (described in the
mode sections above). The point is: the tier number is a simple integer, and
your question generator uses it to look up how hard to make things.
...
The *dashboard* can show the tier (as a label like "Level 3" or a small
indicator), but it should feel informational, not judgmental.

---

## Part 5: The Dashboard

The dashboard is what the user sees after logging in and between sessions. It
...
- Current streak (consecutive days practiced) — e.g., "🔥 4-day streak!"
- Modes available to play (with a "Play" button for each)

**After at least one session:**

- Last session summary (mode, score, accuracy)
- Accuracy trend over the past 7 days (a simple chart — even a row of colored bars works)
- Per-mode breakdown: accuracy in each mode they've played

**After several sessions (Sprint 2+):**

- Personal bests per mode
- Current difficulty tier per mode (optional, subtle)
- Suggestion: "Your weakest mode is Fill It In — try a session?"
...
Here's a map of the routes (URLs) your Flask application will need. You don't
have to build them all at once — they're organized by which sprint they belong to.
...
**A note about how sessions work in Flask:** Each game session needs to track
...
This document covers everything you need to know about what you're building
and how it works under the hood. The companion document `G4_Project_Planning.md`
covers when you're building it, how the team works together, and what you're
graded on.

Between the two documents, you have enough to start Sprint 1 today.

When you get stuck — and you will, everyone does — re-read the relevant section
here, talk to your team, ask your AI assistant, and ask your instructor. In
that order.
